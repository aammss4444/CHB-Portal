...
async def main():
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(Institution).filter(Institution.code == '4455'))
        inst = result.scalars().first()
        if not inst:
            print("Inst 4455 (SOE) not found")
            return
        
        print(f"Found inst {inst.id} ({inst.name}). Deleting...")
        try:
