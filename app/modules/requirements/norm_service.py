"""
norm_service.py — DTE Norm business logic for CHB Portal Step 1.

Three async functions:
  - resolve_norm      : priority-based norm lookup (COURSE_WISE → GENERAL → 400)
  - create_norm       : insert a single norm with uniqueness check + audit
  - seed_dte_defaults : idempotent bulk-seed of all 5 DTE course categories
"""
...
async def resolve_norm(
    ...
    Priority:
      1. COURSE_WISE norm matching institution + year + course_category
      2. GENERAL norm matching institution + year
      3. HTTP 400 with code NORM_NOT_CONFIGURED
    """
    # Step 1 — course-wise lookup
...
async def create_norm(
    ...
    Enforces uniqueness on (institution_id, academic_year, norm_type, course_category).
    ...
    stmt = select(Norm).where(
        ...
        Norm.course_category == (payload.course_category.value if payload.course_category else None),
    )
...
async def seed_dte_defaults(
    ...
    """
    Seed all 5 DTE course-wise norms for an institution in one call.

    Idempotent: skips categories that already exist, never errors on duplicate.
    Writes one audit_log entry per successfully seeded norm.
    """
    seeded = 0
    skipped = 0
    detail: list[str] = []
