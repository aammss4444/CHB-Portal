...
@router.post(
    ...
    dependencies=[Depends(principal_only)],
)
...
@router.put("/{advertisement_id}", response_model=AdvertisementEnvelope, dependencies=[Depends(principal_only)])
...
@router.post("/{advertisement_id}/submit", response_model=AdvertisementEnvelope, dependencies=[Depends(principal_only)])
